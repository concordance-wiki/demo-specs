...
4. **ARIA roles and states.** Collapsibles carry `aria-expanded` and `aria-controls`, a `details` starts with a named `summary` and nests no link or button, fields carry a label, search forms are named landmarks, and a tab strip uses the tablist pattern. A static checker enforces these on every page, next to the structural rules (one `h1`, headings in order, landmarks, alternatives, unique ids, skip link).
