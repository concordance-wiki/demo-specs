...
A file is not valid UTF-8, a note saved in a single-byte encoding for instance. The file is skipped entirely: nothing of it enters the model, and every link that points to it is reported as a [broken link](broken-link.md). The linter reports it before the file is pushed, and an `.editorconfig` with `charset = utf-8` keeps the mistake from coming back.
