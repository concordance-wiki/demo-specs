...
The small map of the [entity page](../pages/entity-page.md) that shows the closest entities at one hop, all labelled, and never stands alone: a list next to it carries the same information in text. Deliberately modest, six nodes by default and twelve at most; beyond the cap the map draws the nodes kept and says how many neighbours the page has in total, and a page without any neighbour points at the [mentions panel](mentions-panel.md) instead.

## Today

The map is folded behind the last line of the side panel, a small network mark before "See the neighbourhood map · n pages", n counting the nodes the map draws. Opening it turns that line into the head of the map, a ◂ back control before the title "Neighbourhood map" and the name of the page; on a desktop the map then takes the place of the other blocks of the panel, which widens to hold it, so that the text of the page stays readable beside it and the map never occupies the page. On a phone the map opens in the flow of the page under the blocks, on a tablet at the foot of the page. The fold is a native disclosure: it opens without any script and over a local file.

Under its head, the map shows its controls: the distance it is drawn at, "Distance · 1 hop", the one distance the model records, since the neighbourhood is computed at build and a second hop is never computed in the browser; and "Types", a filter of checkboxes, one per type present in the neighbourhood with its count, served all ticked, that hides the nodes, the edges and the rows of a type unticked. The map sits in a frame on the soft surface: the page at its centre is the one node drawn in the accent, the current position, its neighbours are small grey nodes around it. Under the map, a legend reads "existing page" beside a plain rule and "word without a note" beside a dashed one.
...
The map is drawn for at most the configured number of nodes, `site.neighbourhood.size`, six by default and twelve at most. When the model holds more neighbours than that, the map draws the ones kept and one sentence under it says so, "n neighbours in total, more than the map shows"; the list of those neighbours follows. A page without any neighbour draws no map: the notice "No neighbour recorded" links to the mentions panel, where every citation is listed. A [keyword page](../pages/keyword-page.md) carries no link, so its map is drawn from its co-occurrences: the pages and the other words that accompany it most often, six at most, a page as a full node and a word without a note as a dashed one, each counted by the paragraphs it shares with the word.

The map is hidden from assistive technologies and the list under it, headed "The six neighbours · textual equivalent" and closed by the note "Six neighbours at most, always named. Beyond that the map teaches nothing: the list takes over.", is authoritative: the same neighbours in the same order, each entry opening on the accent mark and naming the entity by its title, its type as plain text by the label the profile gives it in the site language ("Keyword" for a word without a note), the nature of the [link](../../objects/inference/link.md) by the relation label read from the page, so that a screen reads "accesses" and the object it reaches reads "is accessed by", and its passage count. The figure points at the list, the list is never hidden, and the page writes no wording of its own into it: what the profile and the project's labels say reaches the reader unchanged. The list is plain text in the served HTML, where the search index reads it with the rest of the page, so a neighbour found there leads to the page as any other word does.
...
5. Without any neighbour, see every citation → [mentions panel](mentions-panel.md)
