...
Left open: the panel beyond two hundred passages. From two hundred mentions on, the page embeds nothing beyond the first twenty and the island fetches the fragment; whether a list of that size stays readable in the panel, or needs a page of its own, is to be decided on a real corpus once one reaches it. The accessibility audit verifies today that the twenty served entries read without JavaScript and that a plain link to the fragment stands for the others.
