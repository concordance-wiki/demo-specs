...
From 1180 px the page has three columns, the tree, the text and the panel. Between 768 and 1179 px the tree folds behind its name above the text and the panel keeps its column. Under 768 px everything reads in one column: the tree folded, the text, then the blocks of the panel as closed disclosures. The tree and every block are native disclosure elements served closed, which the stylesheet keeps open where the layout has room, so that the folding needs no script and the page reads over `file://` and without JavaScript. Every control is a target of at least 40 px and no text goes under 13 px; the chrome uses one type family for the text, headings included, and a monospace one for paths and identifiers only, both shipped with the site.
