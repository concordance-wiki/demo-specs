...
The right panel lists the properties of the meeting, the date, the duration, the space linking to the file tree of the [home](home.md), and the files, "3 grouped", with why the build grouped them under the list, read from the pairs the reconciliation scored, "same folder, same base name, same commit, high textual overlap", else the criterion recorded on the [entity](../../objects/inference/entity.md). The related pages are the [mentions panel](../panels/mentions-panel.md), with the note that a meeting does not enter the model: it brings passages, and sometimes a decision someone took the trouble to write elsewhere. Last, the [neighbourhood map](../panels/neighbourhood-map.md) folded behind its line.
