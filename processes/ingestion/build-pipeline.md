...
5. Type every note and every document through the cascade, compute identifiers (a document keeps its extension), resolve application and domain; produce an [entity](../../objects/inference/entity.md) per note and per document, the metadata of a document as its attributes.
6. Run the source plugins on the typed entities: the contract importers read the contract each API note declares and add its endpoints, their `exposes` [links](../../objects/inference/link.md), the candidate objects and one contract record per contract.
...
9. Produce the [links](../../objects/inference/link.md): written links, frontmatter references, mentions in mapped sections and in prose, co-occurrences per paragraph with the bounded neighbourhood.
...
15. Write the [model](../../objects/inference/model.md) and the build log, then one fragment per entity next to them: the note rendered to sanitised HTML section by section, its written links turned into page links, its plain text and the text of every page of its documents for the search index, the original file and the PDF of each document kept for the page, or the passages of a keyword page. Nothing after this step reads a source.
