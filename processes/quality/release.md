...
7. Let the image workflow build the container image from the tag, compare a [build](../ingestion/build-pipeline.md) inside the container with one outside, measure the image and push it under the version and `latest`; copy its size into the release notes.
